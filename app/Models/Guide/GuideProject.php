...
class GuideProject extends Model
{
    ...
    function indice($prj)
    {
        ...
        $GuideBreadCrumb = new \App\Models\Guide\GuideBreadCrumb();

        $dt = $this->le($prj);
        $sx = $this->header($dt);
        $sx .= $GuideCSS->style($prj);

        $sx .= $GuideBreadCrumb->breadcrumb($dt);

        $sa = $GuideSection->menu($prj);
        $sb = $GuideSection->indice($prj);
        $sx .= bs(bsc($sa, 4) . bsc($sb, 8));

        return $sx;
    }

    function view_guide($sec)
    {
        $GuideSection = new \App\Models\Guide\GuideSection();
        $GuideBreadCrumb = new \App\Models\Guide\GuideBreadCrumb();

        $dts = $GuideSection->find($sec);
        $prj = $dts['sc_project'];
        ...
        $sb = '';
        $sb .= $GuideSection->summary($sec,0);

        $sa = $GuideSection->menu($sec);
        $sx .= bsc($sa, 4);
        $sx .= bsc($sb, 8);
